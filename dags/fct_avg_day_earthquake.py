...
LONG_DESCRIPTION = """
# Описание DAG: fct_avg_day_earthquake

## Цель
Этот DAG предназначен для ежедневной агрегации данных о землетрясениях из слоя `ods` в целевую таблицу `dm.fct_avg_day_earthquake` в дата-warehousе. Основная задача — вычисление среднего значения магнитуды (`mag`) для каждого дня на основе данных из таблицы `ods.fct_earthquake`.

## Контекст
- **Владелец**: SterkhovAS
- **Слой данных**: raw → ods → dm
- **Источник**: Данные о землетрясениях (earthquake)
- **Целевая таблица**: `dm.fct_avg_day_earthquake`
...
3. **Удаление временной таблицы (drop_stg_table_before)**: Удаление старой временной таблицы `stg.tmp_fct_avg_day_earthquake_<дата>`, если она существует.
4. **Создание временной таблицы (create_stg_table)**: Создание временной таблицы с агрегированными данными (дата и средняя магнитуда) за указанный день.
5. **Удаление старых данных в целевой таблице (drop_from_target_table)**: Удаление записей из `dm.fct_avg_day_earthquake`, соответствующих дате из временной таблицы.
6. **Вставка данных в целевую таблицу (insert_into_target_table)**: Перенос агрегированных данных из временной таблицы в `dm.fct_avg_day_earthquake`.
...
SHORT_DESCRIPTION = "Обработка и агрегация данных о землетрясениях по дням"
